...
\vspace{-3ex}
    ...
    \[
    \exists \vg \in \reals^2 \text{ such that }\LIN(\vf) = g_1 f_1 + g_2 f_2 = \vg^T \vf =: \ip{\vg}{\vf} \equiv \vg \bigcdot \vf \quad \forall \vf \in \reals^2.
    \]
...
\item[Geometry] (Euclidean) distance: $\norm{\vf} := \sqrt{f_1^2 + f_2^2}$
...
\begin{align*}
    ...
    & \qquad \text{(sequences that should converge  do); $\reals$ is complete, $\mathbb{Q}$ is not} \\
...
\only<2,3>{\begin{proof}
		Note that 
		$
		\INT : f \mapsto \int_{[0,1]^d} f(\vx) \, \dif \vx$ and $\AVG: f \mapsto \frac 1n \sum_{i=1}^n f(\vx_i)
		$
