...
Can we prove it without referring to a basis for $V$?}
\only<2->{\begin{proof}
...
\only<3->{Otherwise, pick any nonzero $g_\perp \in \{ h \in V : \ip{h}{f} = 0 \ \forall f \in \ker(L)\}$ \alert{(How?)}, i.e., $g_\perp$ is orthogonal to all vectors in $\ker(L)$. 
\only<5>{

Define $\dist\bigl(h,\ker(L)\bigr) := \inf\{ \norm{h - f} : f \in \ker(L)\}$, i.e., the closest $\ker(L)$ comes to $h$.  For any $h \notin \ker(L)$, choose $k_n \in \ker(L)$ such that $\norm{h - k_n}^2 \le \dist^2\bigl(h,\ker(L)\bigr) + 1/n$ for $n = 1, 2, \ldots$.  Then
\begin{align*}
    \norm{k_m - k_n}^2 &= \norm{(k_m - h) - (k_n-h) }^2 = 2\norm{k_m - h}^2 + 2\norm{k_n-h}^2 - \norm{(k_m - h) + (k_n-h) }^2 \\
    &= 2\norm{k_m - h}^2 + 2\norm{k_n-h}^2 - 4\norm{(k_m+k_n)/2-h) }^2 \\
    & \le 2\bigl[\dist^2\bigl(h,\ker(L)\bigr) + 1/m] + 2\bigl[\dist^2\bigl(h,\ker(L)\bigr) + 1/n] - 4\dist^2\bigl(h,\ker(L)\bigr) = 2(1/m+1/n)
\end{align*}
So, $k_n \to k \in V$ due to completeness, $k \in \ker(K)$
}  
...
\begin{frame}{Ther Exists a $g_\perp$ Orthogonal to All of $\ker(V)$}
\begin{lemma}
     If $L: V \to \reals$ is any \alert{continuous} linear functional on the Hilbert space $(V,\ip{\cdot}{\cdot})$, and $\ker(L) := \{f \in V : L(f) = 0\} \ne V$,
 	then there \alert{exists a} $g_\perp \in V$ such that $g_\perp \in \{ h \in V : \ip{h}{f} = 0 \ \forall f \in \ker(L)\}$.
\end{lemma}
