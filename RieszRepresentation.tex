...
\institute{Department of Applied Mathematics \&
	...
	\& Office of Research \\  Illinois Institute of Technology \quad
...
\vspace{-3ex}
    ...
    \]}
    then we can represent $\LIN(f)$ as an \alert{inner product}\uncover<2-4>{: $\exists$ \alert{coefficient} $\vg \in \reals^2$ such that 
    \[
...
\item[Geometry] (Euclidean, crow flies) distance/size/length/norm: $\norm{\vf} := \sqrt{f_1^2 + f_2^2}$ \\
Pythagorean Theorem
...
\begin{align*}
    ...
    & \qquad \text{$\reals$ is complete, $\mathbb{Q}$ is not} \\
